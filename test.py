...
class DynamicGrid(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.text = tk.Text(
            self, wrap="char", borderwidth=0, highlightthickness=0, state="disabled"
        )
    ...
    def add_box(self, color=None):
        bg = (
            color
            if color
            else random.choice(("red", "orange", "green", "blue", "violet"))
        )
        box = tk.Frame(
            self.text, bd=1, relief="sunken", background=bg, width=100, height=100
        )
